...
def get_student():
    ...
    if not student_by_name.empty:
        student_data = student_by_name.iloc[0].to_dict()
    elif not student_by_reg.empty:
        student_data = student_by_reg.iloc[0].to_dict()
    else:
        return jsonify({"message": "Student not found!"}), 404
    ...
    # Prepare chart data
    df['CGPA'] = pd.to_numeric(df['CGPA'], errors='coerce')  # Ensure CGPA is numeric

    cgpa_data = df[["Name", "Register Number", "CGPA"]].dropna().to_dict(orient="records")
    leetcode_data = df[["Name", "Register Number", "No.of.Problems Solve(Leetcode)"]].dropna().to_dict(orient="records")
